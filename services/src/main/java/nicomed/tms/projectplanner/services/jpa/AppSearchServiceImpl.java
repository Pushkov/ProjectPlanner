...
public class AppSearchServiceImpl implements AppSearchService {
    ...
    public AppSearchWrapperDto getSearchResult(String term, Boolean document, Boolean engineer) {
        List<DocumentDto> documentDtoList = documentService.search(DocumentFilter.builder().term(term).build());
        List<EngineerDto> engineerDtoList = engineerService.search(EngineerFilter.builder().term(term).build());
